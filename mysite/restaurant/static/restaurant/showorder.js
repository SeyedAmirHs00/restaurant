...
function normalizeProductTableBody() {
  for (let textNode of textNodesUnder(productTableBody)) {
    if (textNode.nodeValue.replace(/\u00a0/g, "x").trim().length == 0){
      textNode.remove();
    }

  }
}
normalizeProductTableBody();
